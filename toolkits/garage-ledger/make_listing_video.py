"""Generate Etsy listing video for a Garage Ledger variant.

~9 sec, 1080×1080, 30 fps, MP4 (H.264, no audio).

Usage:  python make_listing_video.py <variant>
        python make_listing_video.py all

Output: dist/<slug>/listing/06_video.mp4
"""
...
# OUT is set per-variant inside main()
OUT = None
...
# ──────────────────────────────────────────────────────────────────────
# CONTENT — set per-variant inside main() before render_frame() runs.
# ──────────────────────────────────────────────────────────────────────
TITLE_LINES = []
TAGLINE = ""
INPUT_ROWS = []
...
def build_content_for_variant(variant_key):
    """Build (TITLE_LINES, TAGLINE, INPUT_ROWS, RESULT_ROWS, GRAND_TOTAL_*) for a variant."""
    cfg = VARIANTS[variant_key]
    v = cfg["default_vehicle"]

    if variant_key == "master":
        title_lines = [
            ("GARAGE", "INK", 140),
            ("LEDGER.", "HI_VIS", 140),
        ]
    else:
        # Variants get smaller title + variant subtitle in hi-vis
        title_lines = [
            ("GARAGE LEDGER", "INK", 88),
            (v["build_type"].upper() + ".", "HI_VIS", 100),
        ]

    tagline = cfg["tagline"]

    target_sale_str = f"${v['target_sale_value']:,}" if v['target_sale_value'] else "Keeper rig"

    input_rows = [
        ("Vehicle",            f"{v['year']} {v['make']} {v['model'][:14]}"),
        ("Build type",         v["build_type"]),
        ("Build budget",       f"${v['build_budget']:,}"),
        ("Target sale",        target_sale_str),
        ("Shop rate",          f"${v['shop_rate']}/hr"),
        ("ROI target",         f"{int(v['roi_target_pct']*100)}%"),
    ]

    # Result rows pull from the sample-expenses sum so the video matches what
    # a buyer will see when they open the workbook for the first time.
    samples = cfg.get("sample_expenses", [])
    total = sum(float(qty) * float(unit) for (_, _, _, _, _, qty, unit) in samples)
    diy_value = sum(float(qty) * float(unit) for (_, cat, _, _, _, qty, unit) in samples
                    if cat.startswith("DIY Hours"))
    budget_remaining = v["build_budget"] - total
    true_cost = total + diy_value

    result_rows = [
        ("TOTAL SPENT",       f"${total:,.0f}"),
        ("BUDGET REMAINING",  f"${budget_remaining:,.0f}"),
        ("DIY LABOR VALUE",   f"${diy_value:,.0f}"),
        ("TRUE COST",         f"${true_cost:,.0f}"),
    ]

    return (title_lines, tagline, input_rows, result_rows,
            "INSTANT DOWNLOAD", PRICE_BY_SLUG[cfg["slug"]])
...
def render_one_variant(variant_key):
    global OUT, TITLE_LINES, TAGLINE, INPUT_ROWS, RESULT_ROWS
    global GRAND_TOTAL_LABEL, GRAND_TOTAL_VALUE

    cfg = VARIANTS[variant_key]
    OUT = Path(__file__).parent / "dist" / cfg["slug"] / "listing"
    OUT.mkdir(parents=True, exist_ok=True)

    content = build_content_for_variant(variant_key)
    TITLE_LINES, TAGLINE, INPUT_ROWS, RESULT_ROWS, GRAND_TOTAL_LABEL, GRAND_TOTAL_VALUE = content

    out_path = OUT / "06_video.mp4"
    print(f"\n[{cfg['slug']}] Rendering {N_FRAMES} frames at {FPS} fps -> {DURATION}s")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python make_listing_video.py <variant>|all")
        sys.exit(1)
    arg = sys.argv[1]
    if arg == "all":
        for key in VARIANTS:
            render_one_variant(key)
    else:
        if arg not in VARIANTS:
            print(f"Unknown variant: {arg}")
            sys.exit(1)
        render_one_variant(arg)
